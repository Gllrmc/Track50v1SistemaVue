                    ...
                    <v-dialog v-model="ludialog" max-width="400px">
                        <v-card>
                            ...
                            <v-card-actions>
                                ...
                                <v-btn color="blue darken-1" text @click="guardar">Confirmar</v-btn>
                            </v-card-actions>
                            </v-card>
                    </v-dialog>
                </template>
    ...
    computed: {
        ...
        formTitle () {
            ...
            switch (this.editedIndex){
                case 1: {
                    title = 'Edicion detalle: ' + me.headersregistros[3].text;
                }
            }
            return title
        },
    ...
    methods: {
            ...
            switch(dia){
                case 'lu' : {
                    if (item.luid){
                        ...
                        item.lufhhasta = new Date(new Date(item.lufhdesde).getTime() + luminutos * 60000);
                        ...
                        axios.put('api/Registros/Actualizarhoras',{
                            ...
                            let itemIndex = this.registros.indexOf(item);
                            let editedItem = this.registros[itemIndex];
                            this.registros.splice(itemIndex, 1, editedItem);
                        ...
                        axios.post('api/Registros/Crear',{
                            ...
                            'liquidable': true,
                            'fhdesde':new Date(item.lufhdesde),
                            'fhhasta': new Date(item.lufhhasta),
                            'minutos':luminutos,
                            'latdesde':item.lulatdesde,
                            'longdesde':item.lulongdesde,
                            'lathasta':item.lulathasta,
        ...
        editRegistroFull(item,dia){
            ...
            switch (dia){
                case 'lu' : {
                    debugger
                    me.lufdesde=new Date(item.lufhdesde).toISOString().substr(0,10);
                    me.luhdesde=new Date(item.lufhdesde).toISOString().substr(11,5);
                    me.lufhasta=new Date(item.lufhhasta).toISOString().substr(0,10);
                    me.luhhasta=new Date(item.lufhhasta).toISOString().substr(11,5);
